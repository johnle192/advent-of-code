function checksum(matrix) {
  ...
  for (i = 0; i < matrix.length; i++) {
    var minimum = matrix[i].reduce(function (x, y) {
      return Math.min(x, y);
    });

    var maximum = matrix[i].reduce(function (x, y) {
      return Math.max(x, y);
    });
...
function checkQuotientSum(matrix) {
  var sum = 0;
  for (x = 0; x < matrix.length; x++) {
    var sortedArray = matrix[x].sort(function(a, b){return b-a})
    sum += checkQuotient(sortedArray);
  }
  console.log(sum);
}

function checkQuotient(array) {
